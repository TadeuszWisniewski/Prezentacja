...
{
    public class NowyKodCPVViewModel : JedenViewModel<KodCpv> //wszystkie zakladni dziedzicza po WorkSpaceViewModel
    {
        #region Konstruktor
        public NowyKodCPVViewModel()
            : base("Kod CPV")
        {
            item = new KodCpv();
        ...
        {
            if (Aktywny == false || Nazwa.IsNullOrEmpty() || Nazwa.StartsWith(' ') || Kod.IsNullOrEmpty() || Kod.StartsWith(' '))
            {
                return 0;
            }
            else
